...
function [R_est, R_init, S_vec] = DESC(Ind, RijMat, params, R_orig, ErrVec)
...
%                wijk((cum_ind(l)+1):cum_ind(l+1)) + params.Gradient.GetStep(grad);
    ...
    if params.make_plots
        svec_errors(end+1) = mean(abs(ErrVec - S_vec));
        R_est = GCW(Ind, AdjMat, RijMat, S_vec);
        [~, MSE_means(end+1),MSE_medians(end+1), ~] = GlobalSOdCorrectRight(R_est, R_orig);
    end
